module MsgPack exposing
    ( MsgPack, Error(..)
    , decode
    , isEmpty, isNil, toBool, toBytes, toDict, toFloat, toInt, toJson, toList, toString
    , encode
    , empty, nil, bin, bool, float, ext, int, string
    , list, object
    , asBytes
    , asString
    )
    ...
    let
        ...
        parseIntBits num accum =
            case num of
                ...
                _ ->
                    parseIntBits (num // 2) (remainderBy 2 num :: accum)
                ...
                |> Maybe.map
                    (\s ->
                        case String.toInt s of
                            ...
                            Just i ->
                                parseIntBits (i // 2) (remainderBy 2 i :: [])
        ...
        expBits =
            (exp + 1023)
                |> (\n -> parseIntBits (n // 2) (remainderBy 2 n :: []))
                |> List.map String.fromInt
                |> String.join ""
                |> String.padLeft 11 '0'
...
toBool msgpack =
    case msgpack of
        ...
        _ ->
            ("Expected MsgPack's Boolean, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
...
toBytes msgpack =
    case msgpack of
        ...
        _ ->
            ("Expected MsgPack's Bin or Extension, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
...
toDict msgpack =
    case msgpack of
        ...
        _ ->
            ("Expected MsgPack's Map, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
    ...
    case msgpack of
        ...
        Text value ->
            case String.toFloat value of
                Nothing -> 
                    ("Conversion failure from MsgPack's Text to Float -" ++ value)
                        |> ConversionFailure
                        |> Err
                Just f ->
                    Ok f
        _ ->
            ("Expected MsgPack's Double, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
    ...
    case msgpack of
        ...
        Text value ->
            case String.toInt value of
                Nothing ->
                    ("Conversion failure from Text to Int -" ++ Debug.toString msgpack)
                        |> ConversionFailure
                        |> Err
                Just i ->
                    Ok i
        ...
        _ ->
            ("Expected MsgPack's Integer, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
            ...
            case mp of
                ...
                Bin value ->
                    --(List.map Json.Encode.int >> 
                        (Json.Encode.list Json.Encode.int) value
                ...
                Vector _ value ->
                    (List.map asJson >> Json.Encode.list identity) value
    in
    case msgpack of
        Map _ _ ->
            asJson msgpack
        ...
        _ ->
            ("Expected MsgPack's Map or Vector, received - " ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
...
toList msgpack =
    case msgpack of
        ...
        _ ->
            ("Expected MsgPack's Vector, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
...
toString msgpack =
    case msgpack of
        ...
        _ ->
            ("Expected MsgPack's Text, received -" ++ Debug.toString msgpack)
                |> ConversionFailure
                |> Err
...
asKey msgpack =
    case msgpack of
        ...
        Boolean value ->
            "__elm-msgpack-bool-" ++ Bool.Extra.toString value ++ "__"
        ...
        bits =
            r.bytes
                |> List.foldl
                    (\byte str ->
                        str
                            ++ String.fromInt (Bitwise.and byte 0x80 |> Bitwise.shiftRightZfBy 7)
                            ++ String.fromInt (Bitwise.and byte 0x40 |> Bitwise.shiftRightZfBy 6)
                            ++ String.fromInt (Bitwise.and byte 0x20 |> Bitwise.shiftRightZfBy 5)
                            ++ String.fromInt (Bitwise.and byte 0x10 |> Bitwise.shiftRightZfBy 4)
                            ++ String.fromInt (Bitwise.and byte 0x08 |> Bitwise.shiftRightZfBy 3)
                            ++ String.fromInt (Bitwise.and byte 0x04 |> Bitwise.shiftRightZfBy 2)
                            ++ String.fromInt (Bitwise.and byte 0x02 |> Bitwise.shiftRightZfBy 1)
                            ++ String.fromInt (Bitwise.and byte 0x01)
                    )
                    ""
            ...
            case mp of
                ...
                Map sz items ->
                    let
                        ...
                        encodeItems itemList acc =
                            case itemList of
                                [] ->
                                    accum

                                ( k, i ) :: rest ->
                                    encodeItems rest (encoder i acc ++ encodeText k acc)
                    in
                    if sz > 0 && sz <= 15 then
                    ...
                    let
                        ...
                        encodeItems itemList acc =
                            case itemList of
                                ...
                                i :: rest ->
                                    encodeItems rest (encoder i acc)
                    in
                    if sz > 0 && sz <= 15 then
